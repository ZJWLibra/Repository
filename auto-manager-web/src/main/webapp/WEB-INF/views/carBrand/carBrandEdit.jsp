				...
				<div class="ibox-content">
					...
					<div class="form-group col-lg-offset-3">
						...
						<div class="form-group" style="margin-left:1px">
							<c:if test="${empty carBrand.brandLogo }">
							<img id="brandLogoImg" src="img/default.png" style="width:150px;height:150px;" />
							</c:if>
							<c:if test="${not empty carBrand.brandLogo }">
							<img id="brandLogoImg" src="<%=res.getString("OSS_URL_PREFIX") %>${carBrand.brandLogo }" style="width:150px;height:150px;" />
							</c:if>
					...
					<form role="form" id="update_form" class="form-horizontal col-lg-6 col-lg-offset-3">
						...
						<div class="form-group">
							...
							<input type="text" placeholder="请输入品牌名称" class="form-control" id="brandName" name="brandName" value="${carBrand.brandName }" />
						</div>
						<div class="form-group">
							<label>品牌描述:</label>
							<input type="text" placeholder="请输入品牌描述" class="form-control" id="brandDes" name="brandDes" value="${carBrand.brandDes }" />
						</div>
						<div class="form-group">
							<label>状态:</label>
							<select class="form-control m-b" id="brandStatus" name="brandStatus">
								<c:if test="${carBrand.brandStatus == 'Y'}">
								<option value="Y" selected="selected">启用</option>
								<option value="N">停用</option>
								</c:if>
								<c:if test="${carBrand.brandStatus == 'N'}">
								<option value="Y">启用</option>
								<option value="N" selected="selected">停用</option>
								</c:if>
							</select>
						</div>
						<input type="hidden" id="brandLogo" name="brandLogo" value="${carBrand.brandLogo }" />
						<div>
							<button class="btn btn-sm btn-primary pull-right m-t-n-xs" style="margin-left:5px" type="button" id="update_submit">
								<strong>保存</strong>
							</button>
							<button class="btn btn-sm btn-default pull-right m-t-n-xs" type="reset">
								<strong>重置</strong>
							</button>
						</div>
					</form>
